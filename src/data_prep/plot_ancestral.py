...
def color_tree_probability(tt, tdf, feature, out_file):
	...
	c_func, cmap, norm = pp.continuousFunc(trait_dict=fit_dict, trait="name", cmap=sns.color_palette("Spectral", as_cmap=True), vmin=0, vmax=1)
...
def plot_ancestral_ax(tt, feature, feature_dict, ax, color, probability=False):
	if probability:
		colors = ["#e3e1d5", "#ff00bc", "#d8dae6"]
		cmap1 = LinearSegmentedColormap.from_list("mycmap", colors)
		c_func, cmap, norm = pp.continuousFunc(trait_dict=feature_dict.to_dict(), trait="name", cmap=cmap1, vmin=0, vmax=1)
	else:
...
def plot_presences(tt, features_file, features_list, out_dir, pastml_dir=None, plot_changepoints=False, fname=None):
	...
	for i, feature in enumerate(features_list):
		...
		if isinstance(feature_dict, pd.Series):
			ax, cmap, norm = plot_ancestral_ax(tt, feature, feature_dict, ax, color, probability=probabilities)
