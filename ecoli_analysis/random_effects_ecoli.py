...
from transmission_sim.ecoli.random_effects import dropbox_dir, find_parents, get_parent_type_info
from transmission_sim.ecoli.random_effects_classes import RandomEffectSite, PhyloLossRandomEffIterative
...
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# ========================================================================
# 	Need to create data array with un-segmented tree,
# 	calculate type indices, parent type indices, test-train data sets
# 	from that, then for running with site effects, need to just make
# 	segmented branches inherit from their un-segmented correlate in the
# 	un-segmented tree.
# ========================================================================
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# ========================================================================
...
def load_data(analysis_name, random_name, use_train_idx):
...
def do_crossval(analysis_name, random_name, est_site=False, use_train_idx=True):
	# -----------------------------------------------------
	# Create and/or load fold-segmented tree file 
	# as data object
	# -----------------------------------------------------
	data, phylo_obj, fold_params, folds, out_folder, analysis_params = load_data(analysis_name, random_name, use_train_idx)
...
def analyze_fit(analysis_name, random_name, est_site=False, est_b0=False):
	...
	# as data object
	# -----------------------------------------------------
	all_data, phylo_obj, RO, analysis_params, analysis_out_dir = load_data_and_RO_from_file(analysis_name)
...
def test(analysis_name, random_name):
	all_data, phylo_obj, RO, params, analysis_out_dir = load_data_and_RO_from_file(analysis_name)
	all_data.addArrayParams(b0=(1, False))
...
if __name__ == "__main__":
	...
	# test(analysis_name, random_name, use_train_idx)
